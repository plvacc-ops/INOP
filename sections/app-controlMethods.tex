...
\cbstart%
\subsubsection{Aircraft maneuvering, ``Direct to'' (DCT)}

As a general rule, in procedural space, flying should take place within published procedures. If necessary, it is possible to issue non-standard commands, such as:
\begin{description}
  \item[INTERCEPT RADIAL (degrees) FROM (vor)]
  \item[AFTER (point) DIRECT TO (point)]
  \item[DIRECT TO (point)]  
\end{description}

A ``DIRECT TO'' command in procedural airspace may only be issued if there is reasonable assurance that the aircraft will not leave controlled airspace (or crew consents to do so) and will be procedurally separated from other traffic.

The ``DIRECT TO'' command is not used for aircraft that do not meet RNP 1 requirements. The exception is ``DIRECT TO'', e.g.\ to a radio beacon in order to perform a conventional procedure.
\cbend%
